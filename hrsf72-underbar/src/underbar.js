...
//_.reject ([1,2,3], function(num) {return num % 2 === 0;});
  // Produce a duplicate-free version of the array.
  _.uniq = function(array) {
    var tempResult =[];
      _.each(array, function(item){
        if (!tempResult.includes(item)) {
  ...
  /*_.uniq = function(array) {
    ...
    for (var i = 0; i < array.length; i++) {
      if (!tempResult.includes(array[i])) {
...
*/
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(val, index, coll) {
    	 // add return value of iterator a new array
    	result.push(iterator(val, index, coll));
    } )
    return result;
